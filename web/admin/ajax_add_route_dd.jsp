...
<sql:query dataSource="${ds}" var="result">
        SELECT * FROM address_detail WHERE add_id NOT IN (${param.ids});
</sql:query>

<c:forEach var="data" items="${result.rows}">${data.shop_name_or_house_name}-${data.road},</c:forEach>
:
<c:forEach var="r" items="${result.rows}">${r.add_id},</c:forEach> 